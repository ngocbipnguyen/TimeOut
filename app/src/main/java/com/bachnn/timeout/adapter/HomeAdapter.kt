...
class HomeAdapter(private val clickListener: (AppInfo) -> Unit) : ListAdapter<AppInfo, HomeAdapter.ViewHolder>(AppInfoDiffCallback()) {
    ...
    class ViewHolder(private val binding: AppInfoItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(appInfo: AppInfo,  clickListener: (AppInfo) -> Unit) {
            binding.appInfoIcon.setImageDrawable(appInfo.drawable)
